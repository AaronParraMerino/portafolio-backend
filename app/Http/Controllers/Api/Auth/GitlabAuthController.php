...
class GitlabAuthController extends ProviderOAuthController
{
    ...
    public function syncRepos(Request $request): JsonResponse
    {
        ...
        $result = $this->gitlabRepositorySyncService->syncForUsuario((int) $user->id_usuario);

        return match ($result['status'] ?? 'error') {
            'success' => response()->json($result),
            'not_linked' => response()->json($result, 404),
            'missing_token' => response()->json($result, 422),
            'insufficient_scope' => response()->json($result, 422),
    ...
    public function detectedRepos(Request $request): JsonResponse
    {
        ...
        if ($refreshResponse = $this->refreshGitlabReposIfRequested((int) $user->id_usuario, $refresh)) {
    ...
    public function repoLanguages(Request $request): JsonResponse
    {
        ...
        $result = $this->gitlabRepositorySyncService->fetchRepoLanguagesForUsuario(
            (int) $user->id_usuario,
    ...
    public function attachDetectedReposToProject(Request $request): JsonResponse
    {
        ...
        $result = $this->gitlabRepositorySyncService->attachDetectedReposToProject(
            (int) $user->id_usuario,
    ...
    private function refreshGitlabReposIfRequested(int $usuarioId, bool $refresh): ?JsonResponse
    {
        if (! $refresh) {
            return null;
        }

        $sync = $this->gitlabRepositorySyncService->syncForUsuario($usuarioId);
        ...
        return match ($sync['status'] ?? 'error') {
            ...
            'insufficient_scope' => response()->json($sync, 422),
    ...
    private function getDetectedReposForUsuario(int $usuarioId, bool $countOnly = false): mixed
    {
        ...
        if ($countOnly) {
            return $query
                ->select(['pr.id_proyecto', 'urv.validado', 'pc.permitir_participantes_sin_validacion'])
                ->get()
                ->filter(function ($row) {
